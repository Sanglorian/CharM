...
| `species-extra.yaml` | The other 11 species (Human, Baldanar, Cervitaur, Fetchling, Grynloc, Hoddydod, Quickling, Rook, Selkmay, Spriggan, Whom) — generated by `generate-species`. Together with the above, all 25 source species are in |
| `feats.yaml` | All 133 feats from Player Options (every text field verbatim), generated by `generate-feats`. The pool for the level-1 + even-level feat slots. Fifteen feats carry curated mechanical rules — defenses (with 11th/21st scaling), Perception, Heal, HP, initiative, plus the choice feats Skill Training, Skill Focus, Weapon/Armor/Shield Proficiency and Weapon Focus/Specialization (scoped selects); the rest are text-only |
...
| `kits.yaml` | The 6 hand-wired kits (`Theme`) + the "Has Kit" marker and "Feats and Kits" house-rule elements + the shared `Discipline Access` rows |
| `kits-extra.yaml` | The other 19 kits (generated by `generate-kits`): Dabbles-in-… multiclass kits, the Embodies/Worships/Sculpts/Conjures/Brews/etc. kits — Theme + L1/5/10 features + associated-discipline access. Together: all 25 source kits |
| `deities.yaml` | The four gods (`Deity`) referenced by the "Worships the God of …" kits |
| `deities-named.yaml` | The 13 named setting gods of the Outlaw Kingdoms (`Deity`, generated by `generate-deities`) — Title/Symbol/Portfolio/Favored Weapon + verbatim lore |
| `backgrounds.yaml` | The 46 optional creation backgrounds (`Background`, generated by `generate-backgrounds`) — verbatim Associated Skills + the shared 3-way benefit note |
| `arts.yaml` | The 80 arts — practices & incantations (`Ritual`, generated by `generate-arts`) — Level/Skill/Category/Art Type/Market Price/Component Cost/Time/Duration + verbatim rules body. Plug into the engine's rituals/practices machinery (listed as learnable/purchasable arts) |
| `variants.yaml` | The 8 player-facing optional rules (`Variant`, generated by `generate-variants`) — verbatim Description; surfaced as on/off toggles in the Campaign Settings panel. Bonus Feats carries build rules (Keen Defenses + a Weapon Focus / Focused Caster pick at L11) |
...
Compiles to **2337 elements across 40 types**, no warnings.
...
in full to level 29), the two kit disciplines (Born to Run, Radiant Dawn), all 25
