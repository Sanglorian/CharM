...
| `feats.yaml` | All 133 feats from Player Options (every text field verbatim), generated by `generate-feats`. The pool for the level-1 + even-level feat slots. Nine feats carry curated mechanical rules (defenses + their 11th/21st scaling, Perception, Heal, HP, initiative, and a Skill Training select); the rest are text-only |
